...
void testDefaultPrecision() {
    ...
    mpf_class f("1.5");
    mp_bitcnt_t defaultPrec = gmpxx_defaults::this_thread::get_precision();
    assert(defaultPrec == f.get_prec());
    std::cout << "Default precision: " << f.get_prec() << " test passed." << std::endl;

    gmpxx_defaults::this_thread::set_precision(1024);
    defaultPrec = gmpxx_defaults::this_thread::get_precision();
    assert(defaultPrec == 1024);
    mpf_class g("2.5");
    std::cout << "Set and get precision: " << g.get_prec() << " test passed." << std::endl;

    gmpxx_defaults::this_thread::set_precision(512);
    defaultPrec = gmpxx_defaults::this_thread::get_precision();
    mpf_class h("1.5");
...
void testInitializationAndAssignmentString() {
    ...
    mpf_class e(inputHexValue, gmpxx_defaults::this_thread::get_precision(), 16);
    ...
    gmpxx_defaults::this_thread::set_base(16);
    // Testing initialization with a hexadecimal number using a constructor
    mpf_class f;
    e = inputHexValue;
    assert(Is_mpf_class_Equals(e, expectedHexValue, false, 12, gmpxx_defaults::this_thread::get_base()));
    std::cout << "Constructor initialization with hexadecimal '" << expectedHexValue << "' test passed." << std::endl;
    gmpxx_defaults::this_thread::set_base(10);
...
void test_mpf_class_constructor_precision() {
    ...
    assert(f1.get_prec() == gmpxx_defaults::this_thread::get_precision());
#else
    ...
    f7_2 = y;
#if defined ___GMPXX_THREADSAFE_ONLY___
    assert(f7_2.get_prec() == gmpxx_defaults::this_thread::get_precision());
#else
...
#if defined ___GMPXX_THREADSAFE_ONLY___
    mpf_init2(f, gmpxx_defaults::this_thread::get_precision());
#else
...
void test_mpf_class_const_pi() {
    ...
    gmpxx_defaults::this_thread::set_precision(prec * 2);
...
#if defined ___GMPXX_THREADSAFE_ONLY___
        gmpxx_defaults::this_thread::set_precision(medium_prec);
#else
