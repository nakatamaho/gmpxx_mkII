...
| `mpf_class` RAII | Done through Phase 5 | Owns `mpf_t`; supports default/precision/integral+precision/double/string/raw-`mpf_t` construction, copy/move, assignment including integral, double, and string assignment, compound assignment, precision access, string conversion, stream I/O, raw GMP access, and swap. |
