...
The project is currently at Phase 1.5 of the v2.0.0 rewrite. The implemented
...
arithmetic, compound assignment, and the first scalar expression
optimizations.
...
| Test coverage | Present through Phase 1.5 | Thirteen CTest targets cover ABI traits, numeric equivalence, allocation counts, alias safety, thread-local default precision, scalar arithmetic, scalar allocation counts, compound assignment, long-width dispatch, precision policy, unary simplification, and power-of-two fusion. |
...
| `unary_expr<Op, X>` | Stores operand by `const&`, implements `operand()`, `suggested_prec_impl()`, `contains_address()`, and `eval_to_prec()` | Uses the L1 lifetime policy. Do not save expression trees in `auto`; evaluate immediately with `.eval()` or initialize/assign an `mpf_class`. `-(-x)` is represented as a `pos_op` expression node. |
...
| Operation tags | `add_op`, `sub_op`, `mul_op`, `div_op`, `neg_op`, `pos_op` | Direct wrappers over GMP `mpf_*` arithmetic. Integer scalar paths use `_ui` fast paths when possible, with `mpf_mul_2exp`/`mpf_div_2exp` fusion for powers of two. Double scalar paths use one temporary `mpf_class`. |
...
| Binary `-` | Done through Phase 1 | `mpf_class`, expression, and scalar combinations except scalar/scalar |
| Binary `*` | Done through Phase 1.5 | `mpf_class`, expression, and scalar combinations except scalar/scalar; integer power-of-two scaling uses `mpf_mul_2exp` |
| Binary `/` | Done through Phase 1.5 | `mpf_class`, expression, and scalar combinations except scalar/scalar; `mpf / integer-power-of-two` uses `mpf_div_2exp` |
| Unary `-` | Done through Phase 1.5 | `-mpf_class`, `-expression`, and `-(-x)` simplification |
...
| `GMPXX_MKII_NOPRECCHANGE` build | Present | The same thirteen tests pass when expression construction precision is the thread-local default instead of max operand precision. |
| Environment override check | Present manually | `GMPXX_MKII_DEFAULT_PREC=1024 ctest --test-dir build --output-on-failure` passes. |
| Clang coverage | Present for Phase 1 | Clang 21.1.8 passed both default and `GMPXX_MKII_NOPRECCHANGE=ON` Phase 1 builds; Phase 1.5 rerun is pending. |
| TSan coverage | Present for T4 | ThreadSanitizer build passes `test_thread_safety`. |
| ASan/UBSan coverage | Present for Phase 1 | GCC 15.2.0 AddressSanitizer/UndefinedBehaviorSanitizer build passed all ten Phase 1 tests; Phase 1.5 rerun is pending. |

## Verified Build Matrix

| Compiler / Build | Result | Notes |
|---|---:|---|
| GCC 15.2.0, default | Pass | All thirteen tests pass. |
| GCC 15.2.0, `GMPXX_MKII_NOPRECCHANGE=ON` | Pass | All thirteen tests pass. |
| GCC 15.2.0, `GMPXX_MKII_TEST_LLP64_PATH` | Pass | Long-width dispatch tests pass with the slow path forced. |
| Clang 21.1.8, default | Previous pass | All ten Phase 1 tests passed; Phase 1.5 rerun is pending. |
| Clang 21.1.8, `GMPXX_MKII_NOPRECCHANGE=ON` | Previous pass | All ten Phase 1 tests passed; Phase 1.5 rerun is pending. |
| GCC 15.2.0, TSan | Pass | `test_thread_safety` passes. |
| GCC 15.2.0, ASan/UBSan | Previous pass | All ten Phase 1 tests passed; Phase 1.5 rerun is pending. |
...
Phase 1.5 uses the L1 expression-node storage policy for `mpf_class` leaves and
