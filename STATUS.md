...
| `test_io_and_strings` | Present | `get_str()`, `set_str()`, `to_string()`, raw `print_mpz`/`print_mpq`/`print_mpf`, raw GMP pointer stream input/output, wrapper stream input/output, legacy basic iostream syntax, expression stream output, failure safety, precision preservation, base flags, legacy-style partial input consumption, width/fill adjustment, and GMP-allocated string freeing. |
