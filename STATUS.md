...
| Benchmarks | Present | CMake builds the eager benchmark source layout for `00_Rdot`, `01_Raxpy`, `02_Rgemv`, and `03_Rgemm`, including native `mpf_t`, original `gmpxx.h`, `mkII`, `mkII_NOPRECCHANGE`, and OpenMP target variants where present. `benchmarks/run_benchmarks.sh` records logs and `benchmarks/plot.py` generates summary and per-kernel plots. |
