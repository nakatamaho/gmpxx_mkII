...
| Remaining GMP C++ binding compatibility | Mutable upstream-style `mpq_class` numerator/denominator references, member/static integer helper forms, and full upstream `gmp_randclass(gmp_randalg_t, ...)` constructor compatibility | Tracked in the GMP C++ binding checklist above. These are compatibility surface gaps, not expression-template core gaps. |
