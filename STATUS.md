...
| Examples | Present | Five CMake-built examples demonstrate basic mpf arithmetic, `sqrt`, Newton iteration for `sqrt(2)`, Gauss-Legendre iteration for `pi`, and an Aberth root finder for a degree-10 integer-coefficient polynomial implemented with real-valued complex pairs using the v2.0.0 wrapper precision policy. |
| Benchmarks | Present | CMake-built benchmark drivers cover the legacy `00_Rdot`, `01_Raxpy`, `02_Rgemv`, and `03_Rgemm` kernels in three variants: `gmpxx_mkII`, original `gmpxx.h`, and hand-written `mpf_t`; `benchmarks/run_benchmarks.sh` records logs and `benchmarks/plot.py` generates summary and per-kernel plots. |
