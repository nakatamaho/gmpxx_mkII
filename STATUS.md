...
| `mpz_class` | Integer/string/wrapper construction, copy/move, expression construction/assignment, wrapper assignment, compound assignment, `%=` support, scalar conversion/fits queries, `get_str()`, `set_str()`, `to_string()`, stream I/O, `get_mpz_t()`, `contains_address()`, `swap()`, and `sgn()` | `mpz / mpz` uses `mpz_tdiv_q`; `%=` uses `mpz_tdiv_r`. `mpz_class(mpf_class)` and `mpz_class(mpq_class)` use GMP's truncating conversion semantics. `set_str()` and stream extraction parse into a temporary and leave the object unchanged on failure. No-base string APIs use the wrapper default base. |
