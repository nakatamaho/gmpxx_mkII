...
void _Rgemm(int64_t m, int64_t k, int64_t n, const mpf_t alpha, const mpf_t *A, int64_t lda, const mpf_t *B, int64_t ldb, const mpf_t beta, mpf_t *C, int64_t ldc) {
    ...
    for (int64_t j = 0; j < n; ++j) {
        for (int64_t i = 0; i < m; ++i) {
            mpf_mul(C[i + j * ldc], C[i + j * ldc], beta); // C[i + j*ldc] = beta * C[i + j*ldc]
        }
    }
    ...
    for (int64_t i = 0; i < m; ++i) {
        for (int64_t j = 0; j < n; ++j) {
            ...
            for (int64_t l = 0; l < k; ++l) {
                mpf_mul(temp, A[i + l * lda], B[l + j * ldb]); // temp = A[i + l*lda] * B[p + j*ldb]
                mpf_add(sum, sum, temp);                       // sum += temp
            }
            mpf_mul(sum, sum, alpha);
            mpf_add(C[i + j * ldc], C[i + j * ldc], sum); // C[i + j*ldc] += sum
        }
    }
