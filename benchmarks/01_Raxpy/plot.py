...
for file_path in file_paths:
    ...
    pattern = re.compile(r'/usr/bin/time\s+(\./Raxpy_gmp_[\w]+)\s+\d+\s+\d+\nElapsed time:\s+([\d.]+)\s+s\nMFLOPS:\s+([\d.]+)')

    # Extract data using the defined pattern
    data = pattern.findall(''.join(lines))

    # Organize data, removing './Raxpy_gmp_' from the operation names
    operations = [op.replace('./Raxpy_gmp_', '') for op, _, _ in data]
    ...
    filename = f'singlecore_operations_{os_cpu_model_filename}_{dim}_{prec}.pdf'
    plt.savefig(filename)
    filename = f'singlecore_operations_{os_cpu_model_filename}_{dim}_{prec}.png'
    plt.savefig(filename)
    plt.close()
