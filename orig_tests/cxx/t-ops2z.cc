...
void checkz() {
    ...
    mpz_class ret;
    try {
        ret = factorial(-mpz_class(3));
        ASSERT_ALWAYS(0);
    } catch (std::domain_error &) {
    }
    try {
        ret = mpz_class::factorial(-2);
        ASSERT_ALWAYS(0);
    } catch (std::domain_error &) {
    }
    try {
        ret = factorial(mpz_class(1) << 300);
        ASSERT_ALWAYS(0);
    } catch (std::bad_alloc &) {
    }
    ...
    try {
        ret = primorial(-mpz_class(3));
        ASSERT_ALWAYS(0);
    } catch (std::domain_error &) {
    }
    try {
        ret = mpz_class::primorial(-5);
        ASSERT_ALWAYS(0);
    } catch (std::domain_error &) {
    }
    try {
        ret = primorial(mpz_class(1) << 300);
        ASSERT_ALWAYS(0);
    } catch (std::bad_alloc &) {
    }
